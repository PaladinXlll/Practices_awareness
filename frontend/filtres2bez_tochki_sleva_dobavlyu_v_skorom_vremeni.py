...
# ==============================================================================
# КОНФИГУРАЦИЯ ИНТЕРФЕЙСА (PIXEL PERFECT ПАЛИТРА И НАСТРОЙКИ)
# ==============================================================================
...
COLOR_ITEM_BG = "#bda058"       # Цвет подложки плашек элементов (чуть темнее фона)

# ==============================================================================
# БАЗОВЫЕ КОМПОНЕНТЫ И СТИЛИЗОВАННЫЕ ПЛАШКИ
# ==============================================================================

class FilterCategoryButton(QPushButton):
    """Кнопка главной категории верхнего уровня с круглым маркером и стрелкой"""
...
class StaticListElement(QWidget):
    """ Элемент списка (плашка) с возможностью вложенного меню """
    def __init__(self, text, has_arrow=False, sub_items=None, parent=None):
        ...
        # Сама плашка элемента
        self.row_widget = QFrame()
        self.row_widget.setObjectName("ItemFrame")
        
        # Стилизация плашки под дизайн макета
        self.row_widget.setStyleSheet(f"""
            QFrame#ItemFrame {{
        ...
        self.row_layout.setContentsMargins(10, 6, 10, 6)
        ...
        # Реализация вложенного меню (для Семестров внутри конкретного Учебного года)
        self.sub_menu_widget = None
        if has_arrow and sub_items:
            self.sub_menu_widget = ExpandedSubMenu(sub_items, is_nested=True)
            self.sub_menu_widget.setVisible(False)
            self.main_vbox.addWidget(self.sub_menu_widget)
            
            self.row_widget.setCursor(Qt.CursorShape.PointingHandCursor)
            # Переопределяем нажатие мыши на всю плашку для раскрытия подменю
            self.row_widget.mousePressEvent = self.toggle_sub_menu
...
# ==============================================================================
# ИНТЕГРИРОВАННЫЙ КОМПОНЕНТ КАЛЕНДАРЯ (БЕЗ ЛИШНИХ ОКОН)
# ==============================================================================

class CalendarSubMenu(QWidget):
    """ Виджет календаря, встроенный прямо в дерево фильтра """
    ...
    def __init__(self, parent=None):
        ...
        # Левый вертикальный маркер открытой секции
        self.line_indicator = QFrame()
        self.line_indicator.setFixedWidth(10)
        self.line_indicator.setStyleSheet(f"""
            QFrame {{ 
                background-color: {COLOR_ACCENT_LINE}; 
                border-radius: 5px; 
            }}
        """)
        ...
        # Выпадающий список выбора месяцев
        ...
        # Кнопка быстрой установки текущей даты
    ...
    def update_calendar_grid(self):
        ...
        for col, day in enumerate(days_headers):
            ...
            lbl.setStyleSheet(f"color: {color}; font-family: '{FONT_FAMILY}'; font-weight: bold; font-size: 11px;")
...
# ==============================================================================
# СТРУКТУРНЫЕ КОНТЕЙНЕРЫ СПИСКОВ
# ==============================================================================

class ExpandedSubMenu(QWidget):
    """ Контейнер для отображения вложенных списков с левым маркером-линией """
    def __init__(self, items, has_item_arrows=False, is_nested=False, parent=None):
        super().__init__(parent)
        self.main_layout = QHBoxLayout(self)
        
        # Сдвиг контента вправо для соблюдения иерархии вложенности
        left_margin = 30 if is_nested else 15
        self.main_layout.setContentsMargins(left_margin, 2, 0, 5)
    ...
    def add_static_item(self, text, has_item_arrows):
        # Если это глубоко вложенные подпункты семестров, добавляем им круглый маркер
        if not has_item_arrows and text in ["Зимний", "Летний", "1 полугодие", "2 полугодие"]:
            text = f"●  {text}"
            
        # Для категорий Учебного года передаем список вложенных семестров
        sub_items = ["Зимний", "Летний"] if has_item_arrows else None
        
        item_widget = StaticListElement(text, has_arrow=has_item_arrows, sub_items=sub_items)
        self.list_layout.addWidget(item_widget)


# ==============================================================================
# ГЛАВНОЕ ОКНО ИНТЕРФЕЙСА ПАНЕЛИ
# ==============================================================================

class CalendarGoldFilterWidget(QWidget):
    """ Главный виджет панели фильтрации """
    def __init__(self):
        super().__init__()
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # --- ДИНАМИЧЕСКАЯ ГЕНЕРАЦИЯ СТРУКТУРЫ (READ-ONLY) ---
        
        # 1. Мероприятия (Опечатки успешно исправлены согласно промпту)
        ...
        self.create_filter_section(
            "Преподаватели", 
            ["Агашина Анастасия Евгеньевна", "Денисова Анна Алексеевна", "Томилова Анна Владимировна"], 
            is_open=False
        )
        
        # 3. Интегрированная Дата (Календарь разворачивается плавно внутри списка)
        self.create_calendar_section("Дата", is_open=False)
        
        # 4. Учебный год (Древовидная иерархическая структура)
        self.create_filter_section(
            "Учебный год", 
            ["2024/2025", "2025/2026", "2026/2027"], 
            is_open=False, 
            has_item_arrows=True
        )
        
        # 5. Контроль
        self.create_filter_section(
            "Контроль", 
            ["Наумушкина Н.С.", "Кучина А.А.", "Сметанина К.В."], 
            is_open=False
        )
    ...
    def create_calendar_section(self, title, is_open=False):
        ...
        # Пример обработки клика на дату
